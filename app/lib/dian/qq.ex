defmodule Dian.QQ do
  ...
  def get_message_by_id(id) do
    case get("/get_msg", query: [message_id: id]) do
      {:ok, %{status: 200, body: %{"status" => "ok", "data" => data}}} -> {:ok, data}
      error -> error
    end
  end
  ...
  def send_private_msg(user_id, group_id, content) do
    case post("/send_private_msg", %{user_id: user_id, group_id: group_id, message: content}) do
      {:ok, %{status: 200, body: body}} -> {:ok, body}
      error -> error
    end
  end
